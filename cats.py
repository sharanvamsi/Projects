...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def f(p1):
        p1 = [lower(i) for i in split(remove_punctuation(p1))]
        for x in subject:
            for y in p1:
                if x == y:
                    return True
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    count = 0
    for x in range(0, len(source_words)):
        if x < len(typed_words): 
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    y = diff_function(typed_word, word_list[0], limit)
    a = word_list[0]
    for x in range(0, len(word_list)):
        if word_list[x] == typed_word:
            return typed_word
        elif diff_function(typed_word, word_list[x], limit) < y:
            y = diff_function(typed_word, word_list[x], limit)
            a = ''
            a += word_list[x]
    if y > limit:
        return typed_word
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    def f(a, c):
        if a == len(typed) or typed[a] != source[a]:
            upload({'id': user_id, 'progress': (c/len(source))})
            return (c/len(source))   
        else:
            return f(a + 1, c + 1)
    return f(0, 0)
...
def time_per_word(words, timestamps_per_player):
    ...
    new = []
    x = 0
    while x < len(timestamps_per_player):
        new += [[]]
        x += 1
    def f(a, b):
        if (a+1) == len(timestamps_per_player[0]):
            return match(words, b)
        else:
            for z in range(len(timestamps_per_player)):
                b[z] += [timestamps_per_player[z][a+1] - timestamps_per_player[z][a]]
            return f(a+1, b)
    return f(0, new)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    def f(a, b):
        c, d = 0, 0
        if a == len(get_all_times(match)[0]):
            return b
        else:
            c = time(match, 0, a)
